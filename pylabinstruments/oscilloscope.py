...
class OscilloscopeBase(LibraryTemplate, ABC):
    ...
    @parameter_validator(
        ...
        label=lambda text: len(text) <= 32,  # Most oscilloscopes limit label length
    )
    ...
    @parameter_validator(channel=lambda c: c > 0, scale=lambda s: s > 0)
    @visa_exception_handler(default_return_value=None, module_logger=logger)
    ...
    @parameter_validator(channel=lambda c: c > 0, coupling=lambda c: c.upper() in ["AC", "DC", "GND"])
    @visa_exception_handler(default_return_value=None, module_logger=logger)
    ...
    @parameter_validator(trigger_source=lambda s: s.startswith("CH") or s in ["EXT", "LINE", "WGEN"])
    @visa_exception_handler(default_return_value=None, module_logger=logger)
    ...
    @parameter_validator(
        trigger_level=lambda lvl: -1000 <= lvl <= 1000  # Wide range to accommodate various signal levels
    )
    @visa_exception_handler(default_return_value=None, module_logger=logger)
    ...
    @parameter_validator(acquisition_mode=lambda m: m.upper() in ["SAMPLE", "AVERAGE", "ENVELOPE", "PEAK", "HIRES"])
    @visa_exception_handler(default_return_value=None, module_logger=logger)
    ...
    @parameter_validator(length=lambda n: isinstance(n, int) and n > 0)
    @visa_exception_handler(default_return_value=None, module_logger=logger)
    ...
    @parameter_validator(measurement_type=lambda m: isinstance(m, str) and len(m) > 0, channel=lambda c: c > 0)
    ...
    @parameter_validator(
        ...
        label=lambda text: text is None or len(text) <= 32,
    )
    @visa_exception_handler(default_return_value=None, module_logger=logger)
...
class TektronixDPO2000(OscilloscopeBase):
    ...
    def get_measurement_statistics(self, measurement_type: str, channel: int) -> Dict[str, float]:
        ...
        results = {"mean": mean, "min": minimum, "max": maximum, "stddev": stddev}
...
class TektronixMDO3000(OscilloscopeBase):
    ...
    @parameter_validator(start_freq=lambda f: f > 0, stop_freq=lambda f: f > 0)
    @visa_exception_handler(default_return_value=None, module_logger=logger)
